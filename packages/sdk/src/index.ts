...
class Uptime {
  ...
  constructor(
    ...
    if (serviceName.length < 5)
      throw `serviceName should be at least 6 chracters`;
    if (secondsBetweenHeartbeat < 10)
      throw "secondsBetweenHeartbeat should be at least 10 seconds";
    if (secondsBetweenAlerts < 10)
      throw "secondsBetweenAlerts should be at least 10 seconds";
    if (maxAlertsPerDownTime < 1)
      throw "maxAlertsPerDownTime should be at least 1";
    if (!discordWebhook && !(pushoverGroup && pushoverToken))
      throw "Pass either discordWebhook or pushoverGroup + pushoverToken";
    this.alive = true;
  }
      ...
      .then((res) => {
        if (!res.success)
          throw new FetchError(
            "Uptime Monitor: Invalid HOST or application not running"
          );
      })
      .catch((err) => {
        throw new FetchError(
  ...
  private async heartbeat() {
    while (this.alive) {
      try {
        ...
        await sleep(
          (this.secondsBetweenHeartbeat -
            Math.floor(this.secondsBetweenHeartbeat / 2)) *
            1000
        );
      } catch (err) {
  ...
  public async terminate() {
    const resp = await fetch(this.host + "records", {
      method: "POST",
      headers: {
        "Content-type": "application/json",
      },
